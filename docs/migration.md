...
the webhook and provide TLS certs (see [Helm Deploy → Webhook Configuration](./helm_deploy.md#webhook-configuration-optional)).
...
!!! warning
    The Helm chart upgrade applies the CRD before the new operator pod
    rolls out. If you flip `webhook.enabled=true→false` while v1-stored
    objects exist, there is a window in which the conversion block is
    already gone but the old pod is still reconciling — any reconcile that
    crosses versions during that window will fail to deserialize.
